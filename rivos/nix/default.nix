{
  ...
  officialRelease ? {
    version = "15.0.6";
    sha256 = "sha256-Zt/+J6AToJmimlsZPd1huuET6EkNYa6GKgkUFw0Gco8=";
  },
  # i.e.:
  # {
  #   version = /* i.e. "15.0.0" */;
...
in
  ...
  ("must specify `gitRelease` or `officialRelease`"
    + (lib.optionalString (gitRelease != null) " — not both")); let
...
in let
  ...
  majorVersion = lib.versions.major releaseInfo.release_version;
  ...
  tools = lib.makeExtensible (tools: let
    callPackage = newScope (tools // {inherit stdenv cmake ninja libxml2 python3 release_version version monorepoSrc packageVendor buildLlvmTools;});
    mkExtraBuildCommands0 = cc: ''
      rsrc="$out/resource-root"
      mkdir "$rsrc"
      ln -s "${cc.lib}/lib/clang/${majorVersion}/include" "$rsrc"
  ...
  in {
    ...
    clangNoLibcxxUseLLVM = wrapCCWith rec {
      cc = tools.clang-unwrapped;
      libcxx = null;
      bintools = bintools';
      extraPackages =
  ...
  libraries = lib.makeExtensible (libraries: let
    callPackage = newScope (libraries // buildLlvmTools // {inherit stdenv cmake ninja libxml2 python3 release_version version monorepoSrc packageVendor;});
  in {
    ...
    compiler-rt =
      /*
         if stdenv.hostPlatform.isAndroid
      then
      */
    ...
    */
    ...
    flang-rt = callPackage ./flang/flang-rt.nix {
      inherit llvm_meta;
    };
    ...
    libcxxabi = let
      ...
      # RIVOS: we _do_ want compiler-rt here, and for now we can disregard darwin.
      stdenv_ = overrideCC stdenv buildLlvmTools.clangNoLibcxxUseLLVM;
    in
    ...
    libcxx = callPackage ./libcxx {
      inherit llvm_meta;
      stdenv = overrideCC stdenv buildLlvmTools.clangNoLibcxxUseLLVM;
    };
