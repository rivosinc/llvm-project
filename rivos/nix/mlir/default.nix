# From https://github.com/NixOS/nixpkgs/pull/163878
{
  ...
  bins = map (n: "mlir-" + n) (runners
    ++ [
      "linalg-ods-yaml-gen"
      "tblgen" # needed for cross
      "lsp-server"
      "opt"
      "pdll"
      "reduce"
      "translate" # misc utilities
    ]);
in
  stdenv.mkDerivation rec {
    ...
    cmakeFlags =
      [
        ...
        "-DCMAKE_PREFIX_PATH=${libllvm}/lib/cmake/llvm"
      ]
    ...
    postPatch = lib.optionalString enableRunners ''
      ...
      substituteInPlace lib/ExecutionEngine/CMakeLists.txt --replace 'if(NOT TARGET ''${LLVM_NATIVE_ARCH})' 'if (0)'
    '';

    postBuild = ''
      ninja ${lib.concatStringsSep " " bins}
    '';
